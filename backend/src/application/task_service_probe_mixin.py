...
from ..llm_service.client.openai_chat_client import OpenAIChatClient
from ..llm_service.client.openai_client import OpenAICLient, OpenAIClientBaseOptions
from .model_provider_profiles import requires_tencent_secrets
...
class TaskServiceModelProbeMixin:
    ...
    def _probe_model_config(self, config: dict[str, Any]) -> None:
        ...
        provider = self._video_provider_name(config) if kind == "video" else self._model_provider_name(config)
        if requires_tencent_secrets(kind, provider):
            if not config.get("secret_id") or not config.get("secret_key"):
                raise ValueError(f"腾讯云{self._model_kind_label(kind)}模型未配置 SecretId 或 SecretKey")
        elif not api_key:
        ...
        try:
            ...
            raise ValueError(
                f"{self._model_kind_label(kind)}模型嗅探失败"
                f"（实际调用模型：{model}）：{detail}"
            ) from exc

    def _openai_client(self, config: dict[str, Any], api_key: str, model: str) -> Any:
        options = OpenAIClientBaseOptions(
            api_key=api_key,
            base_url=str(config.get("endpoint") or "").strip() or None,
            model=model,
        )
        return OpenAIChatClient(options) if self._model_provider_name(config) in {"dashscope", "tencent"} else OpenAICLient(options)
    ...
    def _probe_image(self, config: dict[str, Any], api_key: str, model: str) -> None:
        result = self._generate_provider_image(
            {**config, "api_key": api_key, "model": model}, "生成一张简单的纯色测试图", ratio="1:1"
        )
    ...
    def _probe_video(self, config: dict[str, Any], api_key: str, model: str) -> None:
        client = self._video_task_client(config)
        if client is None:
            raise ValueError("当前视频模型不支持异步任务嗅探")
        provider = self._video_provider_name(config)
        if provider == "tencent":
        ...
        created = client.create_video_task(
            ...
            ratio="16:9",
            resolution="720p" if provider == "dashscope" else "480p",
            seconds=3,
        ...
        try:
            ...
            try:
                if self._video_provider_supports_cancellation(config):
    ...
    def _probe_audio(self, config: dict[str, Any], api_key: str, model: str) -> None:
        result = self._generate_provider_audio(
            {**config, "api_key": api_key, "model": model}, "模型连接测试"
        )
        if not result.get("url") and not result.get("content"):
            raise RuntimeError("音频模型没有返回有效结果")
