...
class ShortDrama(ORMBase):
    ...
    episode_count: Mapped[int] = mapped_column(Integer, default=25, server_default="25", comment="Requested episode count; read by screenplay expansion and storyboard planning, set only when the project is created.")
    ...
    expanded_script_min_chars: Mapped[int] = mapped_column(Integer, default=5_000, server_default="5000", comment="Minimum characters required for the expanded screenplay; read by bootstrap expansion and set when the project is created.")
    expanded_script_max_chars: Mapped[int] = mapped_column(Integer, default=10_000, server_default="10000", comment="Hard character ceiling for the expanded screenplay; read by bootstrap expansion and set when the project is created.")
    shot_script_max_chars: Mapped[int] = mapped_column(Integer, default=400, server_default="400", comment="Maximum characters in each generated storyboard script segment; read by decomposition and set when the project is created.")
