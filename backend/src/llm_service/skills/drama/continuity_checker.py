...
class ContinuityCheckerSkill(BaseSkill):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "current_episode_script": {"type": "string"},
            "previous_summary": {"type": "string"},
        },
        "required": [
            "story_bible",
            "known_state",
            "current_episode_script",
            "previous_summary",
        ],
        "additionalProperties": False,
    }
    instruction = """
检查当前剧本并输出问题列表与修订建议，覆盖：人物是否知道不该知道的信息、
人物动机、时间线、地点移动、道具状态、伏笔回收、关系变化、重复冲突和结尾钩子。
问题按 critical、major、minor 分级；没有问题时明确返回通过。
""".strip()
