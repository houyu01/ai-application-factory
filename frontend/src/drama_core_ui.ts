...
import type { ApiProject, DramaAsset, DramaAssetImageHistory, DramaAssetKind, DramaAssetMetadata, DramaAssetVariant, DramaPlacement, DramaPromptAssetType, DramaPromptNode, DramaShot, DramaShotVersion, GenerationTask, Project, VoicePreset } from './models.js';
import './drama_image_viewer.js';
...
export function dramaAssets(project: ApiProject) { return project.assets || []; }
...
export function resolveMediaUrl(value?: string | null) { if (!value) return ''; if (/^(https?:|data:|blob:)/.test(value)) return value; if (value.startsWith('/api/')) return `${rt().apiBaseUrl}${value.slice(4)}`; return `${rt().apiBaseUrl}${value.startsWith('/') ? value : `/${value}`}`; }
...
export function latestDramaTask(project: ApiProject, type: string, resourceId?: string) { return [...(project.tasks || [])].reverse().find(task => task.type === type && (resourceId === undefined || task.resource_id === resourceId)); }
export function dramaTaskRunning(project: ApiProject, type: string, resourceId?: string) { return latestDramaTask(project, type, resourceId)?.status === '生成中'; }
...
export function enhanceDramaShotEditor(project: ApiProject, shot: DramaShot) {
  ...
  if (!editor) return;
  const videoTask = latestDramaTask(project, 'shot_video', shot.id);
  ...
  const referenceGrid = referencePanel?.querySelector<HTMLElement>('.drama-reference-grid');
  if (referenceGrid) {
    const selectedReferences = references.map(reference => {
      const asset = assets.get(reference.asset_id);
      return { reference, asset };
    });
    referenceGrid.innerHTML = selectedReferences.length
      ? selectedReferences.map(({ reference, asset }) => `<div class="drama-reference-card ${!asset || asset.status !== '生成成功' || !asset.image_url ? 'is-missing' : ''}"><span class="drama-reference-thumb">${asset?.image_url ? `<img src="${rt().escapeHtml(resolveMediaUrl(asset.image_url))}" data-drama-image-preview="${rt().escapeHtml(resolveMediaUrl(asset.image_url))}" data-drama-image-label="${rt().escapeHtml(asset.name)}" alt="" />` : '⚠'}</span><span><b>${rt().escapeHtml(asset?.name || reference.label || '未命名素材')}</b><small>${dramaKindLabel(reference.asset_type)} · ${asset?.status || '素材不存在'}</small></span></div>`).join('')
      : '<div class="drama-reference-empty">暂无参考素材</div>';
  }
  ...
  if (!references.length) {
    ...
    status.className = `drama-reference-status ${missingReferences.length ? 'has-warning' : 'is-ready'}`;
    status.dataset.dramaReferenceStatus = 'true';
    status.innerHTML = missingReferences.length
      ? `<span>⚠ 有 ${missingReferences.length} 个参考素材不可用，生成视频前请先生成图片或重新选择参考图。</span><button type="button" class="ghost compact" data-drama-auto-match>自动匹配参考图</button>`
      : `<span>✓ 当前参考素材已就绪（${references.length} 项）</span><button type="button" class="ghost compact" data-drama-auto-match>自动匹配参考图</button>`;
  ...
  if (videoPanel && !videoPanel.querySelector('[data-drama-version-panel]')) {
    const versions = shot.versions || [];
    const versionPanel = document.createElement('section');
    versionPanel.className = 'drama-shot-version-panel';
    versionPanel.dataset.dramaVersionPanel = 'true';
    versionPanel.innerHTML = `<div class="section-title"><div><h3>生成任务版本</h3><p>每次生成都会保留独立的视频版本和模型任务状态。</p></div><span>${versions.length} 个版本</span></div>${versions.length ? versions.map(version => `<div class="drama-shot-version-item"><div><b>版本 ${version.version_no}</b><small>${dramaTime(version.created_at)}${version.provider_task_id ? ` · ${rt().escapeHtml(version.provider_task_id)}` : ''}</small></div><span class="status ${version.status === '生成中' ? 'running' : version.status === '生成成功' ? 'success' : version.status === '生成失败' ? 'failed' : ''}">${version.status === '生成中' ? `生成中 ${version.progress || 0}%` : rt().escapeHtml(version.status)}</span>${version.video_url ? `<a class="drama-download-button" href="${rt().escapeHtml(resolveMediaUrl(version.video_url))}" target="_blank" rel="noreferrer">打开</a>` : ''}</div>`).join('') : '<p class="muted">暂无视频版本，点击生成视频后会创建版本记录。</p>'}`;
    videoPanel.append(versionPanel);
  }
  const videoButton = document.querySelector<HTMLButtonElement>('#drama-generate-shot-video');
  if (videoButton) setTaskButtonLoading(videoButton, videoTask, '▣ 生成视频', shot.status === '生成中' && !videoTask);
...
export function applyDramaGenerationLoading(project: ApiProject) {
  ...
  const allAssetLoading = kindAssets.some(asset => dramaTaskRunning(project, 'asset_image', asset.id) || asset.status === '生成中' || (asset.variants || []).some(variant => dramaTaskRunning(project, 'asset_variant_image', variant.id) || variant.status === '生成中'));
  ...
  if (shot) {
    ...
    const videoButton = document.querySelector<HTMLButtonElement>('#drama-generate-shot-video'); if (videoButton) setGenerationButtonLoading(videoButton, dramaTaskRunning(project, 'shot_video', shot.id) || (!latestDramaTask(project, 'shot_video', shot.id) && shot.status === '生成中'), '▣ 生成视频');
  ...
  const allPromptButton = document.querySelector<HTMLButtonElement>('#drama-generate-all-prompts'); if (allPromptButton) setGenerationButtonLoading(allPromptButton, dramaShots(project).some(item => dramaTaskRunning(project, 'shot_prompt', item.id)), '✣ 批量生成提示词');
  ...
  if (shot) enhanceDramaShotEditor(project, shot);
}
export function scheduleDramaTaskRefresh(project: ApiProject) { pollDramaTasks(project); }
document.addEventListener('click', event => { const target = event.target instanceof HTMLElement ? event.target : null; const button = target?.closest<HTMLButtonElement>('[data-drama-generate-asset],[data-drama-generate-variant],#drama-generate-shot-prompt,#drama-generate-shot-video,#drama-generate-all-prompts,#drama-generate-all-videos,[data-drama-generate-all-assets],[data-placeholder-generate],#node-generate'); if (!button || button.disabled) return; const idleText = button.id === 'node-generate' ? '▣ 生成节点视频' : button.textContent?.trim() || '生成'; setGenerationButtonLoading(button, true, idleText); }, true);
...
export function syncDramaShotReferencePanel(project: ApiProject) {
  const grid = document.querySelector<HTMLElement>('.drama-reference-grid');
  const shot = dramaSelectedShot(project);
  if (!grid || !shot) return;
  const references = (shot.prompt_rich || []).filter(node => node.type === 'reference');
  const signature = references.map(node => `${node.asset_id}:${node.asset_type}:${node.image_url || ''}`).join('|');
  if (grid.dataset.referenceSignature === signature) return;
  grid.dataset.referenceSignature = signature;
  grid.replaceChildren();
  if (!references.length) {
    grid.innerHTML = '<div class="drama-reference-empty">暂无参考素材</div>';
    return;
  }
  const assets = new Map(dramaAssets(project).map(asset => [asset.id, asset]));
  grid.innerHTML = references.map(node => {
    const asset = assets.get(node.asset_id);
    const imageUrl = resolveMediaUrl(asset?.image_url || node.image_url);
    const label = asset?.name || node.label || '占位图';
    const typeLabel = node.asset_type === 'placeholder' ? '占位图' : dramaKindLabel(node.asset_type);
    const image = imageUrl
      ? `<img src="${rt().escapeHtml(imageUrl)}" data-drama-image-preview="${rt().escapeHtml(imageUrl)}" data-drama-image-label="${rt().escapeHtml(label)}" alt="${rt().escapeHtml(label)}" />`
      : `<span class="drama-reference-no-image">${typeLabel === '占位图' ? '占位图' : '暂无图片'}</span>`;
    return `<div class="drama-reference-card"><span class="drama-reference-thumb">${image}</span><span><b>${rt().escapeHtml(label)}</b><small>${rt().escapeHtml(typeLabel)}</small></span></div>`;
  }).join('');
}

export function dramaAssetImageMarkup(asset: DramaAsset | DramaAssetVariant) {
  ...
  return `<div class="drama-asset-placeholder">${type === 'character' ? '♙' : type === 'scene' ? '✦' : type === 'placeholder' ? '▱' : '◆'}</div>`;
}
...
export function dramaAssetCard(project: ApiProject, asset: DramaAsset) {
  ...
  return `<article class="drama-asset-card" data-drama-asset-card="${rt().escapeHtml(asset.id)}" data-asset-name="${rt().escapeHtml(asset.name.toLowerCase())}"><div class="drama-asset-main"><div class="drama-asset-image">${dramaAssetImageMarkup(asset)}</div><div class="drama-asset-body"><div class="drama-asset-heading"><div><h3>${rt().escapeHtml(asset.name)} <small>${rt().escapeHtml(asset.id.slice(-8))}</small></h3><span>${dramaKindLabel(asset.type)} · 基础形态</span></div><div class="drama-asset-card-tools">${asset.image_url ? `<a class="drama-icon-button" href="${rt().escapeHtml(resolveMediaUrl(asset.image_url))}" download target="_blank" rel="noopener" title="下载图片">${icon('download')}</a>` : ''}<button class="drama-icon-button" data-drama-edit-asset="${rt().escapeHtml(asset.id)}" title="编辑">${icon('edit')}</button><button class="drama-icon-button danger" data-drama-delete-asset="${rt().escapeHtml(asset.id)}" title="删除">${icon('trash')}</button></div></div><div class="drama-asset-badges"><span class="status ${dramaStatusClass(asset.status)}"${statusTitle ? ` title="${rt().escapeHtml(statusTitle)}"` : ''}>${rt().escapeHtml(dramaStatusText(asset.status))}</span><span class="drama-asset-form-badge">基础形态</span></div>${failureReason ? `<div class="drama-asset-error"><b>生成失败原因：</b>${rt().escapeHtml(failureReason)}</div>` : ''}${dramaAssetVoiceField(asset)}<p class="drama-asset-alias"><b>别名：</b>${rt().escapeHtml(asset.name)} / ${rt().escapeHtml(asset.name)}</p><p class="drama-asset-prompt"><b>图片提示词：</b>${rt().escapeHtml(asset.prompt || '等待生成素材提示词')}</p><details class="drama-asset-variants" ${variants.length ? '' : 'hidden'}><summary>展开其他形态 <span>${variants.length} 个其他形态</span></summary><div class="drama-asset-variant-list">${variants.map(variant => dramaAssetVariantCard(asset, variant)).join('')}</div></details></div></div><div class="drama-asset-actions"><button class="small-btn" data-drama-generate-asset="${rt().escapeHtml(asset.id)}">${asset.status === '生成中' ? '生成中…' : `${icon('sparkle')}<span>生成图片</span>`}</button><button class="ghost compact" data-drama-upload-asset="${rt().escapeHtml(asset.id)}">${icon('upload')}<span>上传${dramaKindLabel(asset.type)}</span></button>${variantAction}${dramaImageHistoryButton(asset)}</div></article>`;
}
...
export function openDramaPlaceholderModal(project: ApiProject) {
  ...
  const history = () => dramaAssets(project).filter(asset => asset.type === 'placeholder' && asset.metadata?.shot_id === shot.id).sort((a, b) => String(b.updated_at || '').localeCompare(String(a.updated_at || '')));
  const render = () => {
    ...
    // The generated footer must belong to the flex dialog so it remains visible
    // while the canvas and asset columns scroll inside the modal body.
...
export async function loadDramaDetail(id: string, retry = 0) { const main = document.querySelector('main'); if (!main) return; dramaViewState.projectId = id; try { await rt().loadVoicePresets(); const response = await fetch(`${rt().apiBaseUrl}/projects/${id}`); if (!response.ok) throw new Error(`HTTP ${response.status}`); const project = await response.json() as ApiProject; main.innerHTML = dramaDetailMarkup(project); rt().bindDramaWorkspace(project); const shot = dramaSelectedShot(project); if (shot) setupDramaRichPromptEditor(project, shot); applyDramaGenerationLoading(project); scheduleDramaTaskRefresh(project); } catch (error) { rt().toast('短剧详情加载失败'); console.error(error); } }
export async function deleteDramaProject(projectId: string, fromDetail = false) {
  if (!window.confirm('删除短剧后，分镜、素材、任务和历史视频记录都会被永久删除，确定继续吗？')) return;
  try {
...
export async function dramaPost(path: string) {
  const response = await fetch(`${rt().apiBaseUrl}${path}`, { method: 'POST' });
  if (!response.ok) {
...
export async function dramaRunTask(path: string, successMessage: string) { try { const task = await dramaPost(path); if (successMessage) rt().toast(successMessage); if (activeDramaProject && task?.id) { activeDramaProject.tasks = [...(activeDramaProject.tasks || []).filter(item => item.id !== task.id), task]; applyDramaGenerationLoading(activeDramaProject); scheduleDramaTaskRefresh(activeDramaProject); } return task; } catch (error) { rt().toast(`任务创建失败：${error instanceof Error ? error.message : '请检查素材是否已准备好'}`); console.error(error); return null; } }
...
export function openDramaImageHistoryModal(asset: DramaAsset | DramaAssetVariant, label: string) {
  const history = asset.image_history || [];
  const modal = document.createElement('div');
  modal.className = 'modal-backdrop';
  modal.innerHTML = `<div class="modal drama-image-history-modal"><button class="close" aria-label="关闭">×</button><div class="modal-head"><h2>${rt().escapeHtml(label)} · 图片历史</h2><p>每次生成都会保留历史版本，可以下载或查看。</p></div><div class="drama-image-history-grid">${history.map((item, index) => { const url = resolveMediaUrl(item.url); return `<a class="drama-image-history-item" href="${rt().escapeHtml(url)}" target="_blank" rel="noopener"><div>${url ? `<img src="${rt().escapeHtml(url)}" alt="版本 ${index + 1}" />` : '暂无图片'}</div><span>版本 ${index + 1}</span><small>${rt().escapeHtml(dramaTime(item.generated_at))}</small></a>`; }).join('') || '<p class="muted">暂无图片历史</p>'}</div></div>`;
  document.body.append(modal);
  modal.querySelector('.close')?.addEventListener('click', () => modal.remove());
  modal.addEventListener('click', event => { if (event.target === modal) modal.remove(); });
}

export function bindDramaAssetDrawer(project: ApiProject) {
  document.querySelector('[data-drama-generate-all-assets]')?.addEventListener('click', event => {
    ...
    const jobs = assets.flatMap(asset => [
      dramaRunTask(`/projects/${project.id}/assets/${asset.id}/image`, ''),
      ...(asset.variants || []).map(variant => dramaRunTask(`/projects/${project.id}/assets/${asset.id}/variants/${variant.id}/image`, '')),
    ]);
    void Promise.all(jobs).then(() => rt().toast(`已创建 ${assets.length} 个${dramaKindLabel(dramaViewState.assetPanel || 'prop')}素材的图片任务`));
  });
  ...
  document.querySelectorAll<HTMLElement>('[data-drama-delete-asset]').forEach(element => element.addEventListener('click', async event => { event.preventDefault(); event.stopImmediatePropagation(); const asset = dramaAssets(project).find(item => item.id === element.dataset.dramaDeleteAsset); if (!asset || !window.confirm(`确认删除${dramaKindLabel(asset.type)}“${asset.name}”？`)) return; const response = await fetch(`${rt().apiBaseUrl}/projects/${project.id}/assets/${asset.id}`, { method: 'DELETE' }); if (response.ok) { rt().toast(`${dramaKindLabel(asset.type)}已删除`); void rt().loadDramaDetail(project.id); } else rt().toast('删除失败'); }));
  ...
  document.querySelectorAll<HTMLElement>('[data-drama-delete-variant]').forEach(element => element.addEventListener('click', async event => { event.preventDefault(); event.stopImmediatePropagation(); const asset = dramaAssets(project).find(item => item.id === element.dataset.dramaDeleteVariant); const variant = asset?.variants?.find(item => item.id === element.dataset.dramaVariantId); if (!asset || !variant || !window.confirm(`确认删除形态“${variant.name}”？`)) return; const response = await fetch(`${rt().apiBaseUrl}/projects/${project.id}/assets/${asset.id}/variants/${variant.id}`, { method: 'DELETE' }); if (response.ok) { rt().toast('形态已删除'); void rt().loadDramaDetail(project.id); } else rt().toast('形态删除失败'); }));
