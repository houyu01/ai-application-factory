...
export async function loadDramaDetail(id: string, retry = 0) { const main = document.querySelector('main'); if (!main) return; const scrollState = captureCurrentDramaScrollState(dramaViewState.projectId === id && Boolean(document.querySelector('.drama-detail'))); dramaViewState.projectId = id; try { await rt().loadVoicePresets(); const query = dramaViewState.shotId ? `?shot_id=${encodeURIComponent(dramaViewState.shotId)}` : ''; const response = await fetch(`${rt().apiBaseUrl}/projects/${id}${query}`); if (!response.ok) throw new Error(`HTTP ${response.status}`); const project = await response.json() as ApiProject; suppressExistingModelTaskFailureNotifications(project.tasks || []); main.innerHTML = dramaDetailMarkup(project); rt().bindDramaWorkspace(project); const shot = dramaSelectedShot(project); if (shot) setupDramaRichPromptEditor(project, shot); applyDramaGenerationLoading(project); scheduleDramaTaskRefresh(project); scheduleDramaScrollRestore(scrollState); } catch (error) { rt().toast('短剧详情加载失败'); console.error(error); } }
