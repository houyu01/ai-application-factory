...
import { syncDramaVideoFailure } from './drama_video_failure_ui.js';
...
import { syncDramaBatchVideoCancellation } from './drama_video_batch_cancellation_ui.js';
...
export function enhanceDramaShotEditor(project: ApiProject, shot: DramaShot) {
  ...
  syncDramaVideoPreviewNavigation(videoPanel, project, shot, shotId => { dramaViewState.shotId = shotId; dramaViewState.videoUrl = null; void rt().loadDramaDetail(project.id); });
  syncDramaVideoFailure(videoPanel, shot, videoTask, rt().escapeHtml);
...
export function applyDramaGenerationLoading(project: ApiProject) {
  ...
  if (shot) enhanceDramaShotEditor(project, shot); syncDramaVideoHistoryActions({ apiBaseUrl: rt().apiBaseUrl, project, resolveMediaUrl, loadDramaDetail: rt().loadDramaDetail, toast: rt().toast });
}
...
document.addEventListener('click', event => { const target = event.target instanceof Element ? event.target.closest<HTMLElement>('[data-drama-close-sheet]') : null; if (!target || (target.classList.contains('drama-sheet-backdrop') && event.target !== target)) return; event.preventDefault(); event.stopImmediatePropagation(); dramaViewState.assetPanel = null; const main = document.querySelector('main'); if (main && activeDramaProject) { main.innerHTML = dramaDetailMarkup(activeDramaProject); rt().bindDramaWorkspace(activeDramaProject); } }, true);
...
export function dramaAssetCard(project: ApiProject, asset: DramaAsset) {
  ...
  return `<article class="drama-asset-card" data-drama-asset-card="${rt().escapeHtml(asset.id)}" data-asset-name="${rt().escapeHtml(asset.name.toLowerCase())}"><div class="drama-asset-main"><div class="drama-asset-image">${dramaAssetImageMarkup(asset, project.tasks)}</div><div class="drama-asset-body"><div class="drama-asset-heading"><div><h3>${rt().escapeHtml(asset.name)} <small>${rt().escapeHtml(asset.id.slice(-8))}</small></h3><span>${dramaKindLabel(asset.type)} · 基础形态</span></div><div class="drama-asset-card-tools">${asset.image_url ? `<a class="drama-icon-button" href="${rt().escapeHtml(resolveMediaUrl(asset.image_url))}" download target="_blank" rel="noopener" title="下载图片">${icon('download')}</a>` : ''}<button class="drama-icon-button" data-drama-edit-asset="${rt().escapeHtml(asset.id)}" title="编辑">${icon('edit')}</button><button class="drama-icon-button danger" data-drama-delete-asset="${rt().escapeHtml(asset.id)}" title="删除">${icon('trash')}</button></div></div><div class="drama-asset-badges"><span class="status ${dramaStatusClass(asset.status)}"${statusTitle ? ` title="${rt().escapeHtml(statusTitle)}"` : ''}>${rt().escapeHtml(dramaStatusText(asset.status))}</span><span class="drama-asset-form-badge">基础形态</span></div>${failureReason ? `<div class="drama-asset-error"><b>生成失败原因：</b>${rt().escapeHtml(failureReason)}</div>` : ''}${dramaAssetVoiceField(asset)}<p class="drama-asset-alias"><b>别名：</b>${rt().escapeHtml(asset.name)} / ${rt().escapeHtml(asset.name)}</p><p class="drama-asset-prompt"><b>图片提示词：</b>${rt().escapeHtml(asset.prompt || '等待生成素材提示词')}</p><details class="drama-asset-variants" ${variants.length ? '' : 'hidden'}><summary>展开其他形态 <span>${variants.length} 个其他形态</span></summary><div class="drama-asset-variant-list">${variants.map(variant => dramaAssetVariantCard(asset, variant, project.tasks)).join('')}</div></details></div></div><div class="drama-asset-actions"><button class="small-btn" data-drama-generate-asset="${rt().escapeHtml(asset.id)}">${asset.status === '生成中' ? '生成中…' : `${icon('sparkle')}<span>生成图片</span>`}</button><button class="ghost compact" data-drama-upload-asset="${rt().escapeHtml(asset.id)}">${icon('upload')}<span>上传${dramaKindLabel(asset.type)}</span></button>${variantAction}${dramaImageHistoryButton(asset)}</div></article>`;
}
...
export async function dramaRunTask(path: string, successMessage: string, body?: unknown) { try { const task = await dramaPost(path, body); if (successMessage) rt().toast(successMessage); if (activeDramaProject && task?.id) { activeDramaProject.tasks = [...(activeDramaProject.tasks || []).filter(item => item.id !== task.id), task]; activeDramaProject.assets?.forEach(asset => { if (asset.id === task.resource_id && ['asset_image', 'placeholder_image', 'cover_image'].includes(task.type)) asset.status = task.status; (asset.variants || []).forEach(variant => { if (variant.id === task.resource_id && task.type === 'asset_variant_image') variant.status = task.status; }); }); applyDramaGenerationLoading(activeDramaProject); const sheet = document.querySelector<HTMLElement>('.drama-sheet-backdrop'); if (sheet && dramaViewState.assetPanel) { const wrapper = document.createElement('div'); wrapper.innerHTML = dramaAssetDrawer(activeDramaProject); sheet.replaceWith(wrapper.firstElementChild as HTMLElement); bindDramaAssetDrawer(activeDramaProject); applyDramaGenerationLoading(activeDramaProject); } const editor = document.querySelector<HTMLElement>('.drama-rich-prompt-editor'); if (editor) renderDramaPromptNodes(editor, activeDramaProject, readDramaPromptNodes(editor)); scheduleDramaTaskRefresh(activeDramaProject); } return task; } catch (error) { rt().toast(`任务创建失败：${error instanceof Error ? error.message : '请检查素材是否已准备好'}`); console.error(error); return null; } }
...
/** Capture one still image from a video URL for the first/last-frame editor. */
async function captureDramaVideoFrame(url: string, side: 'first' | 'last'): Promise<string | null> {
  ...
  video.currentTime = side === 'last' ? Math.max(0, video.duration - 0.05) : 0;
  await new Promise<void>((resolve, reject) => {
    video.addEventListener('seeked', () => resolve(), { once: true });
    video.addEventListener('error', () => reject(new Error('video frame unavailable')), { once: true });
  });
...
export function openDramaFrameModal(project: ApiProject) {
  ...
  const choices = (side: 'first' | 'last') => episodeShots.flatMap(item => {
    ...
    const versionVideos = (item.versions || []).filter(version => version.status === '生成成功' && version.video_url).flatMap(version => positions.map(position => ({ url: version.video_url || '', source: 'video', shotId: item.id, videoId: version.id, label: `分镜 ${item.shot_index || ''} · 视频版本 ${version.version_no}`, position })));
    const knownVideoUrls = new Set(versionVideos.map(video => video.url));
    const legacyVideos = (item.historical_videos || []).filter(video => video.url && !knownVideoUrls.has(video.url)).flatMap((video, index) => positions.map(position => ({ url: video.url || '', source: 'video', shotId: item.id, videoId: video.id || String(index), label: `分镜 ${item.shot_index || ''} · 历史视频 ${index + 1}`, position })));
  ...
  const sourcePanel = (side: 'first' | 'last') => `<div class="drama-frame-source-panel" data-frame-source-panel="${side}" hidden><div class="drama-frame-source-head"><div><h4>选择输入${side === 'first' ? '首' : '尾'}帧</h4><p>当前集内其他分镜的首帧和尾帧均可选择</p></div><button type="button" class="drama-frame-source-close" data-frame-library-close="${side}" aria-label="关闭">×</button></div><div class="drama-frame-source-tabs"><button type="button" class="active" data-frame-tab="${side}-library">▧ 可用首尾帧 <b>${choices(side).filter(choice => choice.source !== 'video').length}</b></button><button type="button" data-frame-tab="${side}-video">▣ 从视频提取 <b>${choices(side).filter(choice => choice.source === 'video').length}</b></button></div><div class="drama-frame-choice-grid" data-frame-choice-grid="${side}">${choices(side).map(choice => `<button type="button" class="drama-frame-choice-card" data-frame-choice="${side}" data-frame-url="${rt().escapeHtml(choice.url)}" data-frame-source="${choice.source}" data-frame-shot="${choice.shotId}" data-frame-video="${choice.videoId}" data-frame-position="${choice.position}" data-frame-choice-kind="${choice.source === 'video' ? 'video' : 'library'}"><span class="drama-frame-choice-thumb" ${choice.source === 'video' ? `data-frame-thumb-url="${rt().escapeHtml(choice.url)}" data-frame-thumb-side="${choice.position}"><span>提取中…` : `><img src="${rt().escapeHtml(resolveMediaUrl(choice.url))}" alt="" />`}</span><span class="drama-frame-choice-label">${rt().escapeHtml(choice.label)}<small>${choice.source === 'video' ? `取视频${choice.position === 'first' ? '首' : '尾'}帧` : '已绑定帧'}</small></span></button>`).join('') || '<small>本集暂无可用素材</small>'}</div></div>`;
  const modal = document.createElement('div');
  modal.className = 'modal-backdrop';
  modal.innerHTML = `<div class="modal drama-frame-modal"><button type="button" class="close">×</button><div class="modal-head"><h2>首尾帧 · ${rt().escapeHtml(shot.episode_name || '当前分镜')} / 分镜 ${shot.shot_index || 1}</h2><p>首尾帧只在当前分镜所属集内复用，用于连接相邻分镜。</p></div><div class="drama-frame-editor-grid">${(['first', 'last'] as const).map(side => `<section class="drama-frame-editor-card"><h3>${side === 'first' ? '输入首帧' : '输入尾帧'}</h3><div class="drama-frame-preview">${frames[side]?.url ? `<img src="${rt().escapeHtml(resolveDramaFrameUrl(frames[side]?.url || ''))}" alt="" />` : '尚未设置'}</div><div class="drama-frame-actions"><button type="button" class="ghost compact" data-frame-library="${side}">▧ 从${side === 'first' ? '首' : '尾'}帧库选择</button><label class="ghost compact">↥ 上传图片<input type="file" accept="image/*" data-frame-upload="${side}" hidden /></label></div>${sourcePanel(side)}</section>`).join('')}</div><div class="modal-actions"><button type="button" class="ghost" data-frame-clear>清除首尾帧</button><button type="button" class="primary" data-frame-save>完成</button></div></div>`;
  document.body.append(modal);
  ...
  modal.querySelectorAll<HTMLElement>('[data-frame-thumb-url]').forEach(async thumb => { try { const image = await captureDramaVideoFrame(thumb.dataset.frameThumbUrl || '', thumb.dataset.frameThumbSide as 'first' | 'last'); if (image) thumb.innerHTML = `<img src="${image}" alt="" />`; else thumb.textContent = '无法提取'; } catch { thumb.textContent = '无法提取'; } });
  modal.querySelectorAll<HTMLElement>('[data-frame-choice]').forEach(button => button.addEventListener('click', async () => { const side = (button.dataset.frameChoice || 'first') as 'first' | 'last'; const position = (button.dataset.framePosition || side) as 'first' | 'last'; const preview = button.closest('section')?.querySelector('.drama-frame-preview'); if (!preview) return; try { const image = button.querySelector<HTMLImageElement>('.drama-frame-choice-thumb img')?.src || await captureDramaVideoFrame(button.dataset.frameUrl || '', position); values[side] = { url: image || button.dataset.frameUrl, source: image ? 'frame' : button.dataset.frameSource, shot_id: button.dataset.frameShot, video_id: button.dataset.frameVideo, position }; preview.innerHTML = image ? `<img src="${image}" alt="${side === 'first' ? '首帧' : '尾帧'}" />` : '无法提取帧'; } catch { values[side] = { url: button.dataset.frameUrl, source: button.dataset.frameSource, shot_id: button.dataset.frameShot, video_id: button.dataset.frameVideo, position }; preview.textContent = '无法提取帧'; } }));
  modal.querySelectorAll<HTMLInputElement>('[data-frame-upload]').forEach(input => input.addEventListener('change', () => { const file = input.files?.[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => { const url = String(reader.result || ''); values[input.dataset.frameUpload || 'first'] = { url, source: 'upload' }; const preview = input.closest('section')?.querySelector('.drama-frame-preview'); if (preview) preview.innerHTML = `<img src="${url}" alt="" />`; }; reader.readAsDataURL(file); }));
  modal.querySelector('[data-frame-clear]')?.addEventListener('click', () => { values.first = null; values.last = null; modal.querySelectorAll('.drama-frame-preview').forEach(item => item.textContent = '尚未设置'); });
  modal.querySelector('[data-frame-save]')?.addEventListener('click', async event => {
    ...
    try { const response = await fetch(`${rt().apiBaseUrl}/projects/${project.id}/shots/${shot.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ first_last_frames: values }) }); if (!response.ok) throw new Error(`HTTP ${response.status}`); modal.remove(); rt().toast('首尾帧已保存'); void rt().loadDramaDetail(project.id); }
    catch (error) { button.disabled = false; button.textContent = '完成'; rt().toast('首尾帧保存失败，请稍后重试'); console.error(error); }
  });
  modal.querySelector('.close')?.addEventListener('click', () => modal.remove());
}

export function bindDramaAssetDrawer(project: ApiProject) { bindDramaEpisodeManager(project);
