...
export function dramaTaskRunning(project: ApiProject, type: string, resourceId?: string) { return Boolean(activeDramaTask(project, type, resourceId)); }
...
export function enhanceDramaShotEditor(project: ApiProject, shot: DramaShot) {
  ...
  if (!editor) return;
  const videoTask = activeDramaTask(project, 'shot_video', shot.id) || latestDramaTask(project, 'shot_video', shot.id);
