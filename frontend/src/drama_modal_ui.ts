...
const rt = () => runtime;
export function configureDramaModalRuntime(value: DramaModalRuntime) { runtime = value; configureDramaVideoCancellation({ apiBaseUrl: value.apiBaseUrl, getActiveProject: () => activeDramaProject, getSelectedShot: core.dramaSelectedShot, getVideoTask: (project, shotId) => core.latestDramaTask(project, 'shot_video', shotId), toast: value.toast, reloadProject: value.loadDramaDetail }); }
...
export function ensureDramaDetailToolbar() {
  ...
  expandedScriptButton.addEventListener('click', () => {
    if (dramaViewState.projectId) openDramaExpandedScriptModal({ apiBaseUrl: rt().apiBaseUrl, projectId: dramaViewState.projectId, toast: rt().toast });
  });
  ...
  saveButton.addEventListener('click', async () => {
    ...
    try {
      const requests = [fetch(`${rt().apiBaseUrl}/projects/${project.id}/name`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: projectName }),
      })];
      if (shot) requests.push(fetch(`${rt().apiBaseUrl}/projects/${project.id}/shots/${shot.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
...
function syncPromptVersionDropdown(select: HTMLSelectElement) {
  const dropdown = select.parentElement?.querySelector<HTMLElement>('[data-prompt-version-dropdown]');
...
function ensurePromptVersionDropdown(select: HTMLSelectElement) {
  if (select.parentElement?.querySelector('[data-prompt-version-dropdown]')) {
