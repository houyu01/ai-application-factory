...
export type StorageSettingsResponse = { provider: 'local' | 'tos' | 'cos' | 'oss'; endpoint?: string; bucket?: string; region?: string; prefix?: string; public_base_url?: string; secret_id_masked?: string; secret_key_set?: boolean };
...
export function ensureDramaDetailToolbar() {
  ...
  if (!toolbar) return;
  syncDramaDecompositionBanner(activeDramaProject);
  ...
  globalParamsButton.addEventListener('click', async () => {
    ...
    try {
      const response = await fetch(`${rt().apiBaseUrl}/projects/${dramaViewState.projectId}`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      openDramaGlobalParamsModal(await response.json() as ApiProject);
  ...
  saveButton.addEventListener('click', async () => {
    ...
    const durationValue = Number(document.querySelector<HTMLSelectElement>('#drama-shot-duration')?.value || shot.duration_seconds || 10);
    const editor = document.querySelector<HTMLElement>('.drama-rich-prompt-editor');
    let nodes = editor ? core.readDramaPromptNodes(editor) : [];
    if (!nodes.length) {
      try {
        const storedNodes = JSON.parse(promptInput?.dataset.promptRich || '[]');
        nodes = Array.isArray(storedNodes) ? storedNodes : [];
      } catch { nodes = shot.prompt_rich || []; }
    }
    if (!nodes.length && (promptInput?.value || shot.prompt)) nodes = [{ type: 'text', text: promptInput?.value || shot.prompt }];
    const serialized = core.serializeDramaPromptNodes(project, nodes);
    if (promptInput) {
    ...
    try {
      const response = await fetch(`${rt().apiBaseUrl}/projects/${project.id}/shots/${shot.id}`, {
        ...
        body: JSON.stringify({ title, original_text: originalText, prompt: serialized.prompt, prompt_rich: serialized.nodes, duration_seconds: durationValue }),
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      rt().toast('项目修改已保存');
      void rt().loadDramaDetail(project.id);
...
export function storageProviderStatus(provider: StorageSettingsResponse['provider']) { if (provider === 'local') return '本地文件会保存到 backend/data/media'; return `${({ tos: '火山 TOS', cos: '腾讯 COS', oss: '阿里云 OSS' } as const)[provider]} 已启用`; }
...
export function ensureDramaVideoDownloadButton() { const history = document.querySelector<HTMLElement>('.drama-video-history'); const title = history?.querySelector<HTMLElement>('.section-title'); if (!history || !title || title.querySelector('[data-drama-download]')) return; const selected = history.querySelector<HTMLElement>('.drama-history-item.selected')?.dataset.dramaHistoryUrl || document.querySelector<HTMLVideoElement>('#drama-video-player')?.src || ''; if (!selected) return; const link = document.createElement('a'); link.className = 'ghost compact drama-download-button'; link.dataset.dramaDownload = 'true'; link.href = core.resolveMediaUrl(selected); link.download = 'drama-video.mp4'; link.target = '_blank'; link.rel = 'noopener'; link.textContent = '下载视频'; title.append(link); }
const dramaVideoDownloadObserver = new MutationObserver(ensureDramaVideoDownloadButton);
