...
import { gameGenerationBannerMarkup, gameFlowStatus } from './game_generation_banner_ui.js';
...
async function refreshGameTaskState(game: Game) {
  try {
    ...
    const graphChanged = syncGameTaskPollingUi({ current: game, latest, runtime: rt(), findTask: taskFor, refresh: () => gameDetail(game.id), onRetryGeneration: id => void retryInteractiveGameGeneration(id, true), onStopGeneration: (id, button) => void stopInteractiveGameGeneration(id, button) });
...
async function stopInteractiveGameGeneration(gameId: string, button: HTMLButtonElement) {
  button.disabled = true; button.textContent = '停止中…';
  try {
    const response = await fetch(`${rt().apiBaseUrl}/games/${gameId}/expanded-script/cancel`, { method: 'POST' });
    if (!response.ok) throw new Error(await responseError(response));
    rt().toast?.('已按当前进度保存并停止生成');
    await gameDetail(gameId);
  } catch (error) {
    button.disabled = false; button.textContent = '停止生成';
    rt().toast?.(`停止生成失败：${error instanceof Error ? error.message : '请稍后重试'}`); console.error(error);
  }
}
...
function bindGameEditor(game: Game) {
  const stopGeneration = document.querySelector<HTMLButtonElement>('[data-game-stop-generation]');
  if (stopGeneration) stopGeneration.onclick = () => void stopInteractiveGameGeneration(game.id, stopGeneration);
