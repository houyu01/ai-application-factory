import './style.css'; import './voice_catalog.css'; import './settings_scroll_restore.js'; import { apiBaseUrl } from './desktop_api.js';
...
import { apiKeyVisibilityIcon, applyModelSelect, configureSettingsRuntime, createVoicePreset, loadModelSettings, loadVoicePresets, modelChoices, modelEditorValues, modelSettingsCard, refreshModelSelects, renderModelEditor, saveModelOptions, voiceCatalogCard, voiceOptions, voicePreset } from './settings_ui.js';
...
document.addEventListener('submit', event => {
  ...
  void createVoicePreset({ name, gender, prompt }).then(preset => {
    toast(`已追加音色：${preset.name}`);
  }).catch(error => {
    toast(`追加音色失败：${error instanceof Error ? error.message : '请重试'}`);
