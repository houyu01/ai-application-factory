...
import type { ApiGame, ApiProject, DramaAssetImageHistory, DramaAssetKind, DramaAssetMetadata, DramaAssetVariant, DramaEpisode, DramaPlacement, DramaPromptAssetType, DramaPromptNode, DramaShot, DramaShotVersion, Game, GameAsset, GameEdge, GameNode, GameTask, GenerationTask, Locale, ModelKind, ModelSettings, Project, VoicePreset } from './models.js';
...
const defaultModelSettings: Record<ModelKind, ModelSettings> = {
  language: { kind: 'language', provider: 'ark', endpoint: 'https://ark.cn-beijing.volces.com/api/v3', model: 'doubao-seed-1-6-250615', models: ['doubao-seed-1-6-250615', 'qwen-plus', 'hunyuan-turbos-latest'] },
  multimodal: { kind: 'multimodal', provider: 'ark', endpoint: 'https://ark.cn-beijing.volces.com/api/plan/v3', model: 'doubao-seedream-4-0-250828', models: ['doubao-seedream-4-0-250828', 'qwen-image-2.0', 'hy-image-v3.0'] },
  video: { kind: 'video', endpoint: '', provider: 'ark', create_url: 'https://ark.cn-beijing.volces.com/api/plan/v3/contents/generations/tasks', query_url: 'https://ark.cn-beijing.volces.com/api/plan/v3/contents/generations/tasks/{id}', model: 'doubao-seedance-2.0', models: ['doubao-seedance-2.0', 'sora-2'] },
  audio: { kind: 'audio', provider: 'ark', endpoint: '', create_url: 'https://openspeech.bytedance.com/api/v1/tts_async/submit', query_url: 'https://openspeech.bytedance.com/api/v1/tts_async/query', resource_id: 'volc.tts_async.default', voice: 'BV001_streaming', model: 'volc.tts_async.default', models: ['volc.tts_async.default', 'qwen3-tts-flash', 'mps-sync-dubbing'] },
};
...
configureDramaVideoPreflight({ apiBaseUrl: API_BASE_URL, getActiveProject: () => activeDramaProject, getSelectedShot: dramaSelectedShot, onTaskCreated: (project, task) => { project.tasks = [...(project.tasks || []).filter(item => item.id !== task.id), task]; dramaCore.applyDramaGenerationLoading(project); dramaCore.scheduleDramaTaskRefresh(project); }, toast });
...
function homePage() { const workbenches = [ ['✦', 'drama', ui('dramaDescription')], ['◉', 'interactiveGame', ui('interactiveGameDescription')] ] as const; return `<div class="home-page"><header class="home-page-header"><div><div class="eyebrow">${ui('workspace')}</div><h1>${ui('homeTitle')}</h1><p>${ui('homeDescription')}</p></div></header><section class="workbench-grid">${workbenches.map(([icon,key,description]) => `<button type="button" class="workbench-card workbench-${key}" data-nav="${key}"><span class="workbench-icon">${icon}</span><span class="workbench-copy"><small>${ui('workbench')}</small><strong>${navLabel(key)}</strong><span>${description}</span></span><span class="workbench-arrow" aria-hidden="true">→</span></button>`).join('')}</section></div>`; }
...
function settingsPage() { return `<header class="settings-page-header"><div><div class="eyebrow">${ui('settingsEyebrow')}</div><h1>${ui('settingsTitle')}</h1><p>${ui('settingsDescription')}</p></div></header><section class="settings-grid">${interfaceSettingsCard()}${modelSettingsCard('language', ui('languageModel'), ui('languageModelDescription'), '文', ui('saveModelConfig'))}${modelSettingsCard('multimodal', '图像模型', ui('multimodalModelDescription'), '✧', ui('saveModelConfig'))}${modelSettingsCard('video', '视频模型', '用于生成分镜视频和互动游戏节点视频', '▣', ui('saveModelConfig'))}${modelSettingsCard('audio', '音频模型', ui('audioModelDescription'), '♫', ui('saveModelConfig'))}${voiceCatalogCard()}</section>`; }
...
document.addEventListener('click', event => {
  ...
  const queryUrl = card.querySelector<HTMLInputElement>('[data-model-query-url]')?.value.trim() || ''; const provider = card.querySelector<HTMLSelectElement>('[data-model-provider]')?.value || 'ark'; const region = card.querySelector<HTMLInputElement>('[data-model-region]')?.value.trim() || ''; const secretId = card.querySelector<HTMLInputElement>('[data-model-secret-id]')?.value.trim() || ''; const secretKey = card.querySelector<HTMLInputElement>('[data-model-secret-key]')?.value.trim() || ''; const appId = card.querySelector<HTMLInputElement>('[data-model-app-id]')?.value.trim() || ''; const resourceId = card.querySelector<HTMLInputElement>('[data-model-resource-id]')?.value.trim() || ''; const voice = card.querySelector<HTMLInputElement>('[data-model-voice]')?.value.trim() || ''; const apiKey = card.querySelector<HTMLInputElement>('[data-model-api-key]')?.value.trim() || '';
  ...
  button.disabled = true; button.textContent = '⟳ 正在嗅探调用…';
  void fetch(`${API_BASE_URL}/settings/models`, {
    method: 'PUT', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ kind, model: defaultModel, models, endpoint, create_url: createUrl, query_url: queryUrl, provider, region, secret_id: secretId, secret_key: secretKey, app_id: appId, resource_id: resourceId, voice, api_key: apiKey, generation_concurrency: generationConcurrency }),
  }).then(async response => {
...
document.addEventListener('change', event => { const target = event.target as HTMLSelectElement; if (target?.matches('[data-model-provider]')) { const card = target.closest<HTMLElement>('[data-model-config-card]'); const kind = card?.dataset.modelKind as ModelKind | undefined; const provider = target.value as NonNullable<ModelSettings['provider']>; const profiles: Record<ModelKind, Record<string, Partial<ModelSettings>>> = { language: { ark: { endpoint: 'https://ark.cn-beijing.volces.com/api/v3', model: 'doubao-seed-1-6-250615' }, dashscope: { endpoint: 'https://dashscope.aliyuncs.com/compatible-mode/v1', model: 'qwen-plus' }, tencent: { endpoint: 'https://api.hunyuan.cloud.tencent.com/v1', model: 'hunyuan-turbos-latest' } }, multimodal: { ark: { endpoint: 'https://ark.cn-beijing.volces.com/api/plan/v3', model: 'doubao-seedream-4-0-250828' }, dashscope: { endpoint: 'https://dashscope.aliyuncs.com/api/v1/services/aigc/multimodal-generation/generation', model: 'qwen-image-2.0' }, tencent: { endpoint: 'https://tokenhub.tencentmaas.com/v1/api/image/submit', model: 'hy-image-v3.0' } }, video: { ark: { create_url: 'https://ark.cn-beijing.volces.com/api/plan/v3/contents/generations/tasks', query_url: 'https://ark.cn-beijing.volces.com/api/plan/v3/contents/generations/tasks/{id}', endpoint: '', model: 'doubao-seedance-2.0' }, dashscope: { create_url: 'https://dashscope.aliyuncs.com/api/v1/services/aigc/video-generation/video-synthesis', query_url: 'https://dashscope.aliyuncs.com/api/v1/tasks/{id}', endpoint: '', model: 'happyhorse-1.1-r2v' }, tencent: { endpoint: 'https://mps.tencentcloudapi.com', create_url: 'https://mps.tencentcloudapi.com', query_url: 'https://mps.tencentcloudapi.com', region: 'ap-guangzhou', model: 'Hunyuan:1.5' } }, audio: { ark: { endpoint: '', create_url: 'https://openspeech.bytedance.com/api/v1/tts_async/submit', query_url: 'https://openspeech.bytedance.com/api/v1/tts_async/query', resource_id: 'volc.tts_async.default', voice: 'BV001_streaming', model: 'volc.tts_async.default' }, dashscope: { endpoint: 'https://dashscope.aliyuncs.com/api/v1/services/aigc/multimodal-generation/generation', model: 'qwen3-tts-flash', voice: 'Cherry' }, tencent: { endpoint: 'https://mps.tencentcloudapi.com', region: 'ap-guangzhou', model: 'mps-sync-dubbing', voice: '' } } }; if (kind) { const profile = profiles[kind][provider] || profiles[kind].ark; const model = profile.model || ''; modelSettings[kind] = { ...modelSettings[kind], ...profile, provider, model, models: [model] }; render(); return; } } if (target?.id !== 'storage-provider') return; const provider = target.value as StorageSettingsResponse['provider']; const isLocal = provider === 'local'; document.querySelectorAll<HTMLElement>('.storage-settings-form label').forEach(label => { if (label.querySelector('#storage-endpoint,#storage-bucket,#storage-region,#storage-secret-id,#storage-secret-key')) label.classList.toggle('disabled', isLocal); }); const status = document.querySelector<HTMLElement>('#storage-settings-status'); if (status) status.textContent = storageProviderStatus(provider); }, true);
