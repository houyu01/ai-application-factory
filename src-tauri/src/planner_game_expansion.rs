...
use super::{choices::VIDEO_NODE_TRANSITION_CONTRACT, integer};

const CHOICE_SCREENPLAY_CONTRACT: &str = "选择文案必须是玩家在上一段视频结束时能直接点击的抉择：写具体动作、对话回答、态度立场或信息处置，而不是线索编号、路线名或后果说明。同一剧情段的选择要互斥，并导向不同的即时行动、信息或关系变化。可采用“举手/不举手”“答应/推脱”“交份真实而完整的名单/交份完全虚假的名单”“当然愿意！那我就不客气了/愿意是愿意，只是 300 元有点……”这类互动影视游戏的表达；角色有对白时，优先写符合角色口吻的第一人称回答。“继续推进”只表示线性节点仍需一条边，绝不能直接作为选择文案；应改写成承接画面的具体行动或回答。禁止“线索1”“选项A”“路径1”“路线2”“继续”“下一步”“调查路径”等公式化文案。";

const PROTAGONIST_CONTRACT: &str = "主人公规则（最高优先级，生成任何剧情前必须执行）：先从原始剧本拆解承担核心目标、主要行动和结局后果的人物，确定唯一主人公；若原稿只写事件、场景、群体、反派或泛称，无法可靠拆解出主人公，必须依据原稿的世界与冲突原创补全一名有真实姓名、身份、初始处境和核心目标的主人公。不得只把“玩家”“主角”“某人”当作人物名，不能让整部游戏没有主人公。首次生成时，“【互动剧本总览】”的第一项必须为“主人公：姓名｜身份与初始处境｜核心目标｜玩家扮演关系”；继续扩写时，必须沿用已保存的主人公，不能新建总览或另换主人公。主人公必须以这个姓名出现在 S01 的“出场角色与道具”及剧情正文中，发起或承受每条分支的关键抉择，并在所有结局中有明确结果；后续不得改成无姓名的泛称。";
...
pub(crate) fn game_expansion_prompt(game: &Value) -> String {
    ...
    format!("{prompt}\n\n{CHOICE_SCREENPLAY_CONTRACT}\n\n{VIDEO_NODE_TRANSITION_CONTRACT}\n\n再次确认：{PROTAGONIST_CONTRACT}")
}
...
mod tests {
    ...
    fn expansion_prompt_requires_a_conditioned_branch_screenplay() {
        ...
        for required in [
            ...
            "选择的动作、信息或关系后果必须能在 original_text 中明确定位",
            "主人公：姓名｜身份与初始处境｜核心目标｜玩家扮演关系",
            "不得只把“玩家”“主角”“某人”当作人物名",
        ] {
            assert!(prompt.contains(required), "missing {required}");
        }
        assert!(prompt.matches("不能让整部游戏没有主人公").count() >= 2);
    }
